...
public class HandEvaluator extends Compairer {
    ...
    public int getRank(List<Card> cards) throws CardsNotFoundException{
        // inf here
        ...
        // min will return the value of the best (min rank) 5 card hand within the list of cards provided.
        return Collections.min(rank_list);
    }

    public int getHandAndRank(List<Card> cards) throws CardsNotFoundException{
        // inf here
        IGenerator<List<Card>> cards_gen = Generator.combination(cards).simple(5);
        List<Integer> rank_list = cards_gen.stream().map(comb_cards -> {
            List<String> cards_str = comb_cards.stream().map(
